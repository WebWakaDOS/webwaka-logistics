...
--   - Secure: logi_tracking_tokens are HMAC-signed with 7-day TTL
-- ════════════════════════════════════════════════════════════════════════════
...
CREATE TABLE IF NOT EXISTS logi_order_tracking (
  ...
  status           TEXT    NOT NULL DEFAULT 'PENDING',  -- mirrors logi_delivery_requests.status
...
CREATE INDEX IF NOT EXISTS idx_order_tracking_tenant
  ON logi_order_tracking(tenantId, status);

CREATE INDEX IF NOT EXISTS idx_order_tracking_order
  ON logi_order_tracking(orderId);
...
CREATE INDEX IF NOT EXISTS idx_tracking_tokens_expires
  ON logi_tracking_tokens(expiresAt);
